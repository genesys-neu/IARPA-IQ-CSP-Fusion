...
parser.add_argument('--input', nargs='*', default=['iq'],choices = ['iq', 'nc'],
help='Which data to use as input. Select from: raw IQ data, non-conjugate features, conjugate features.')
parser.add_argument('--iq_slice_len',default=131072, type=int,choices = [131072, 262144, 524288], help='Slice length for processing IQ files') # 32
...
parser.add_argument('--model_folder', help='Location of the model file (with directory)', type=str, default= 'D:/IARPA_DATA/Models_for_Inference/') # For anomaly detection
...
if fusion == False and 'iq' in args.input:
    model_file = args.model_folder + 'IQ_'+str(args.iq_slice_len)+'.pt'
elif fusion == False and 'nc' in args.input:
    model_file = args.model_folder + 'non_conjugate_'+str(args.iq_slice_len)+'.pt'
else:
    model_file = args.model_folder + 'non_conjugate_iq_' + str(args.iq_slice_len) + '.pt'
...
if fusion == False and 'nc' in args.input:
    ...
    for filename in os.listdir(args.data_folder+'IQ/'):
        ...
        for one_sample in iqdata_one_file:
            ...
            row_output = model(torch.from_numpy(one_sample).float().to(device))
            # print("testing111..", row_output)
            row_output = torch.argmax(row_output.squeeze(), dim=0)
            # print("testing222..", row_output)
            row_output = row_output.squeeze().cpu().detach().numpy()
    ...
    for filename in os.listdir(args.data_folder + 'IQ/'):
        ...
        print("The CSP file name: ", args.data_folder+str(args.iq_slice_len)+"/" + filename + '_' + str(args.iq_slice_len) + '*.NC')
        for file_name in glob.iglob(args.data_folder+str(args.iq_slice_len)+"/" + filename + '_' + str(args.iq_slice_len) + '*.NC', recursive=True):
            #print("Coming here")
            one_csp_file = read_one_CSP_feature( file_name, feature_options=args.feature_options)
            one_csp_file = extract_statistic_from_CSP_features_one_col(one_csp_file)
            ...
            row_output = model(torch.from_numpy(one_iq_sample).float().to(device), torch.from_numpy(one_csp_file).float().to(device))
            row_output = torch.argmax(row_output.squeeze(), dim=0)
            row_output = row_output.squeeze().cpu().detach().numpy()
