...
class AlexNet1D(nn.Module): # USED
    def __init__(self, input_dim, output_dim, fusion ='ultimate'):
        ...
        self.fusion_classifier = nn.Sequential(
            nn.Dropout(),
            nn.Linear(524288  , 256), # 2097152 (52 block length) 1048576 (26 block length) 524288  (for 13 block length)
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(256, 256),
            nn.ReLU(True),
...
class FeatureNetCNN(nn.Module): # USED
    def __init__(self, input_dim, output_dim, fusion ='ultimate'):
        ...
        self.hidden1 = nn.Linear(256, 512)  # 32 #512 #input_dim # 86272 scratch #345344 from model
...
class FeatureFusionThree(nn.Module): # USED
    def __init__(self, modelA, modelB, modelC, nb_classes=5, fusion = 'ultimate'):
        ...
        self.hidden1 = nn.Linear(256, 512) # 14592 WHEN training from scratch and incremental fusion; 81792 when restoring the models
...
class RFNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, 5):
            print("Shape in: ", i,  x.shape)
            x = self.relu(self.conv2(x))
            x = self.relu(self.conv3(x))
            x = self.pool(x)
            print("Shape after: ", i, x.shape)
        x = x.view(x.size(0), -1)
        # END OF IT

        x = self.relu(self.hidden1(x)) # was sigmoid
        x = self.drop(x)
        ...
        if self.fusion == 'penultimate':
            ...
            x = self.relu(self.out(x)) # was sigmoid
            return x
